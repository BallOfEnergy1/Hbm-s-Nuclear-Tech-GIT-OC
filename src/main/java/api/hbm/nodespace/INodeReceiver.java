...
public interface INodeReceiver extends INodeHandler {

    public default long transfer(long amount) {
        if(amount + this.getAmount() <= this.getMaxAmount()) {
            this.setAmount(amount + this.getAmount());
            return 0;
        }
        long capacity = this.getMaxAmount() - this.getAmount();
        long overshoot = amount - capacity;
     ...
     * */
    public default INodeReceiver.ConnectionPriority getPriority() {
        return INodeReceiver.ConnectionPriority.NORMAL;
    }
